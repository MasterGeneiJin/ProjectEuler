...
int Euler::PanDigitalMultiples()
{
	...
	{
		if (EulerUtility::hasUniqueDigits(i))
		{
			if (EulerUtility::hasUniqueDigits(i * 2))
			{
				int potentialPanDigital = atoi((std::to_string(i) + std::to_string(i * 2)).c_str());

				if ((EulerUtility::hasUniqueDigits(potentialPanDigital)) && (potentialPanDigital > greatestPanDigitalMultiple))
